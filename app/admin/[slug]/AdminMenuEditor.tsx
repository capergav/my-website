...
export function AdminMenuEditor({
  ...
  const handleSaveTheme = async (updates: {
    ...
    if (error) { showMessage("err", error.message); }
    else { setRestaurant((prev) => (prev ? { ...prev, ...updates } : null)); showMessage("ok", "Theme saved. Refresh the menu page to see changes."); }
    setSaving(false);
  };
  ...
  const handleSaveCategoryNote = async (category: string, note: string) => {
    ...
    if (error) { showMessage("err", error.message); }
    else { setCategoryNotes((prev) => ({ ...prev, [category]: note.trim() })); showMessage("ok", "Category note saved."); }
    setSavingNote(false);
  };
        ...
        <>
              ...
              <div className="tabs-scroll flex gap-2 overflow-x-auto py-3 scrollbar-none">
                {sortedCategories.map((cat) => (
                  <button key={cat} type="button" onClick={() => setActiveCategory(cat)}
                    className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeCategory === cat ? "bg-[var(--accent)] text-white" : "bg-[var(--card-border)]/50 text-[var(--muted)] hover:text-[var(--foreground)]"}`}>
                    {cat}
                  </button>
                ))}
              </div>
          ...
          <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
            ...
            <div className="space-y-4 mt-6">
              {items.map((item) => (
                <div key={item.id} className={`bg-[var(--card)] rounded-2xl border border-[var(--card-border)] overflow-hidden shadow-sm ${item.available === false ? "opacity-60 grayscale" : ""}`}>
                    ...
                    <div className="p-4 flex-1 min-w-0">
                      <div className="flex justify-between gap-2 flex-wrap items-baseline">
                        <div className="flex items-center gap-2">
                          <h3 className="font-serif text-lg font-semibold text-wrap-balance">{item.name}</h3>
                          {item.available === false && (
                            <span className="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">Unavailable</span>
                          )}
                        </div>
                        <span className="font-medium text-[var(--accent)]">${Number(item.price).toFixed(2)}</span>
                      </div>
                      {item.description && <p className="text-[var(--muted)] text-sm mt-1 line-clamp-2 text-wrap-force">{item.description}</p>}
                      <div className="flex flex-wrap gap-2 mt-3 items-center">
                        <AvailabilityToggle
                          ...
                          onChange={async (next) => {
                            ...
                            if (error) { showMessage("err", error.message); }
                            else { showMessage("ok", next ? "Item marked as available." : "Item marked as unavailable."); await refreshMenu(); }
                            setSaving(false);
                          }}
                        />
                        <button type="button" onClick={() => { setEditingItem(item); setAddingNew(false); }}
...
function AvailabilityToggle({ available, onChange }: { available: boolean; onChange: (next: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!available)}
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${available ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" : "bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200"}`}>
...
function CategoryNoteEditor({ category, initialNote, onSave, saving }: { category: string; initialNote: string; onSave: (category: string, note: string) => void; saving: boolean }) {
  ...
  return (
    <div className="mb-6 p-4 rounded-xl border border-[var(--card-border)] bg-[var(--card)]">
      ...
      <textarea value={note} onChange={(e) => setNote(e.target.value)}
        ...
        rows={3} className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)] text-[var(--foreground)] resize-y" />
...
function ThemeDropdowns({ restaurant, onSave, saving }: {
  restaurant: Restaurant | null;
  onSave: (updates: { main_color: string; accent_color: string; background_color?: string | null; font_family?: string | null; font_color?: string | null; name?: string | null; hero_image_url?: string | null }) => void;
  saving: boolean;
}) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
          ...
          <div className="w-full max-w-md rounded-2xl bg-[var(--card)] border border-[var(--card-border)] shadow-xl p-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-sm font-medium text-[var(--foreground)] mb-2">Background color</p>
                <input type="color" value={backgroundColor} onChange={(e) => setBackgroundColor(e.target.value)} className="w-full h-10 rounded-lg border border-[var(--card-border)] bg-[var(--background)] cursor-pointer" />
              </div>
              <div>
                <p className="text-sm font-medium text-[var(--foreground)] mb-2">Font color</p>
                <input type="color" value={fontColor} onChange={(e) => setFontColor(e.target.value)} className="w-full h-10 rounded-lg border border-[var(--card-border)] bg-[var(--background)] cursor-pointer" />
              </div>
              <div>
                <p className="text-sm font-medium text-[var(--foreground)] mb-2">Font family</p>
                <div className="relative">
                  <button type="button" onClick={() => setFontDropdownOpen((o) => !o)}
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 py-1 rounded-lg border border-[var(--card-border)] bg-[var(--card)] shadow-lg z-10 max-h-56 overflow-y-auto">
                      {FONT_OPTIONS.map((opt) => (
                        <button key={opt.value} type="button"
                          ...
                          className={`w-full px-4 py-2.5 text-left hover:bg-[var(--card-border)]/50 ${opt.fontClass} ${fontFamily === opt.value ? "bg-[var(--accent)]/10 text-[var(--accent)]" : ""}`}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
              ...
              <div>
                <p className="text-sm font-medium text-[var(--foreground)] mb-2">Restaurant name</p>
                <input type="text" value={restaurantName} onChange={(e) => setRestaurantName(e.target.value)} placeholder="e.g. La Piazza"
                  className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)] text-[var(--foreground)]" />
              </div>
              <div>
                <p className="text-sm font-medium text-[var(--foreground)] mb-2">Hero image URL</p>
                <input type="url" value={heroImageUrl} onChange={(e) => setHeroImageUrl(e.target.value)} placeholder="https://..."
                  className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)] text-[var(--foreground)]" />
              </div>
              <button type="button"
...
function ItemForm({ item, categories, onSave, onCancel, saving }: { item?: MenuItemRow; categories: readonly string[]; onSave: (p: Partial<MenuItemRow>) => void; onCancel: () => void; saving: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ name: name.trim(), description: description.trim() || null, price: p, image_url: image_url.trim() || null, category: category || "Other", available, chefs_favorite, gluten_free, nut_free, vegan, vegetarian, dairy_free, spicy });
  };
  ...
  const Toggle = ({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) => (
    <div className="flex items-center justify-between gap-2">
      ...
      <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)}
        className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 ${checked ? "bg-[var(--accent)] border-[var(--accent)]" : "bg-[var(--card-border)] border-[var(--card-border)]"}`}>
        <span className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow ring-0 transition-transform mt-0.5 ml-0.5 ${checked ? "translate-x-5" : "translate-x-0"}`} />
      </button>
    </div>
  );

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={(e) => e.target === e.currentTarget && onCancel()}>
      <div className="bg-[var(--card)] rounded-2xl border border-[var(--card-border)] shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
          <h3 className="font-serif text-xl font-semibold mb-4">{item ? "Edit item" : "Add item"}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)] text-wrap-force" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)] text-wrap-force resize-y" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Price *</label>
              <input type="number" step="0.01" min="0" value={price} onChange={(e) => setPrice(e.target.value)} required className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Image URL</label>
              <input type="url" value={image_url} onChange={(e) => setImageUrl(e.target.value)} placeholder="https://..." className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Category</label>
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--background)]">
